...
double c1OnShell(double s_hat, double cos_th) {
    return lam1lam2Sum([=](Helicity l1, Helicity l2) {
        using H = Helicity;
        return onShellHelAmp2(s_hat, cos_th, l1, l2, H::PLUS, H::PLUS) +
               onShellHelAmp2(s_hat, cos_th, l1, l2, H::MINUS, H::MINUS);
    });
}
...
double c3OnShell(double s_hat, double cos_th) {
    return lam1lam2Sum([=](Helicity l1, Helicity l2) {
        using H = Helicity;
        return onShellHelAmp2(s_hat, cos_th, l1, l2, H::MINUS, H::PLUS) +
               onShellHelAmp2(s_hat, cos_th, l1, l2, H::PLUS, H::MINUS);
    });
}
}  // namespace gagatt
